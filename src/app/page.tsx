...
import { Button, Tabs, Tab, Box, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle } from '@mui/material';
...
export default function Home() {
  ...
  const handleFileUpload = (file: File) => {
    ...
    Papa.parse(file, {
      header: true,
      skipEmptyLines: true,
      delimiter: ';', // Set semicolon as delimiter for both formats
      complete: (results) => {
        try {
          // Process the parsed data
          const parsedExpenses = results.data.map((row: any, index: number) => {
            let date, description, amount, merchantCategory;
            ...
            const binding = categoryBindings.find(b => b.merchantCategory === merchantCategory);
  ...
  const handleExpenseUpdate = (updatedExpense: Expense) => {
    setExpenses(prevExpenses => {
      ...
      const selectedExpenses = newExpenses.filter(exp => exp.selected !== false);
      const allValid = selectedExpenses.length > 0 && 
    ...
    <main className="container py-8">
      ...
      >
        ...
        <DialogActions>
          ...
          <Button onClick={handleSaveExpenses} autoFocus variant="contained" color="primary">
            Conferma
          </Button>
        </DialogActions>
      </Dialog>
